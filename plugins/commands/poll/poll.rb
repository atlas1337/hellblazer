  ...
  module Plugins
    ...
    module Poll
      ...
      command(
        ...
        check_poll_table(event.server.id)
        ...
        if started != 1
          ...
          db.execute(
            'UPDATE poll SET started = 1, poll_time = ?, elapsed_time = 0, channel_id = ?, user_id = ? '\
            'WHERE id = 1', time, event.channel.id, event.user.id
          )
          db.close if db
      ...
      command(
        ...
        break unless check_tos(event, event.user.id) == true
        db = SQLite3::Database.new "db/#{event.server.id}.db"
        user = db.execute('SELECT user_id FROM poll WHERE id = 1')
        break unless user == event.user.id || event.server.member(event.user.id).defined_permission?(:administrator)
